...
using Gravitybox.LocationService.EFDAL.Exceptions;
...
namespace Gravitybox.LocationService.EFDAL
{
	...
	internal class LinqSQLFromClauseCollection : List<LinqSQLFromClause>
	{
		...
		internal string GetRealTableName(string parentTable, string field)
		{
			...
			string realTable = string.Empty;
			if (parentTable == "State") realTable = Gravitybox.LocationService.EFDAL.Entity.State.GetTableFromFieldNameSqlMapping(field);
			else if (parentTable == "Zip") realTable = Gravitybox.LocationService.EFDAL.Entity.Zip.GetTableFromFieldNameSqlMapping(field);
		...
		where K : class, IBusinessObjectLINQQuery
	{
			...
			)
		{
			if (string.IsNullOrEmpty(connectionString))
				connectionString = Gravitybox.LocationService.EFDAL.LocationServiceEntities.GetConnectionString();
			...
			)
		{
			return UpdateData(
				...
				getField,
				hasModifyAudit,
				Gravitybox.LocationService.EFDAL.LocationServiceEntities.GetConnectionString());
		}
	...
	internal partial class LinqSQLParser
	{
		...
		private void RemapParentChild()
		{
			...
			foreach (var field in this.FieldList)
			{
				...
				if (_type == ObjectTypeConstants.Table)
				{
					if (clause.TableName == "State") realTable = Gravitybox.LocationService.EFDAL.Entity.State.GetTableFromFieldAliasSqlMapping(field.Alias);
					else if (clause.TableName == "Zip") realTable = Gravitybox.LocationService.EFDAL.Entity.Zip.GetTableFromFieldAliasSqlMapping(field.Alias);
				}
		...
		public static string GetRemappedLinqSql(LinqSQLFromClause tableInfo, string whereClause, LinqSQLFromClauseCollection fromLinkList, ObjectTypeConstants type)
		{
			if (type == ObjectTypeConstants.Table)
			{
				...
				{
					case "State": return Gravitybox.LocationService.EFDAL.Entity.State.GetRemappedLinqSql(whereClause, tableInfo.Alias, fromLinkList);
					case "Zip": return Gravitybox.LocationService.EFDAL.Entity.Zip.GetRemappedLinqSql(whereClause, tableInfo.Alias, fromLinkList);
				}
			}
		...
		private void ParseSelect(string selectClause)
		{
			...
			foreach (string rawField in selectList)
			{
				...
				if (!hasAlias)
				{
					...
					{
						case "State": alias = Gravitybox.LocationService.EFDAL.Entity.State.GetFieldAliasFromFieldNameSqlMapping(field); break;
						case "Zip": alias = Gravitybox.LocationService.EFDAL.Entity.Zip.GetFieldAliasFromFieldNameSqlMapping(field); break;
					}
				}
